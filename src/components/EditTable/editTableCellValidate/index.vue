    ...
    <a-table ref="table" size="default" :columns="columns" :dataSource="value" :pagination="false">
      ...
      >
        <edit-table-cell-validate
          ...
          v-on="cellEvent"
          :ref="'editTableCellValidate' + dataIndex"
        />
      </template>
...
export default {
  ...
  methods: {
    ...
    async callValidateMethod() {
      let validatePromises = [];
      let idx;
      let bol;
      /*  this.value.forEach((item, index) => {
        if (!item.bankAccount || !item.bankCode || !item.bankName) {
          idx = index;
        }
      });
      console.log(this.value, 'value');
      this.columns.forEach((item, index) => {
        // console.log(index, this.columns, 'validateComponent' + index, this.$refs['validateComponent' + index][idx]);
        if (item.title != '操作') {
          this.$refs['validateComponent' + index].forEach(async (item) => {
            if (this.value.length > 0) {
              this.formRefs.push(this.$refs['validateComponent' + index][idx]);
            } else {
              this.formRefs.push(this.$refs['validateComponent' + index][idx]);
            }
            console.log(this.$refs['validateComponent' + index][idx]);
            // await this.$refs['validateComponent' + index][idx].checkValidate();
          });
        }
        // this.$refs['validateComponent' + index][0].checkValidate();
      }); */
      this.value.forEach((item, index) => {
        if (!item.bankAccount || !item.bankCode || !item.bankName) {
          idx = index;
      ...
      await this.columns.forEach((item, index) => {
        if (item.title != '操作') {
          // this.formRefs.push(this.$refs['validateComponent' + index][idx].checkValidate());
          // 调用对应组件的checkValidate方法
          this.$nextTick(() => {
            const ref = this.$refs['editTableCellValidate' + item.dataIndex] || {};
            // console.log(ref[idx], 'ref');
            validatePromises.push(ref[idx]?.checkValidate());
          });
        }
      });
      await Promise.all(validatePromises)
        .then(async (values) => {
          bol = true;
          // await this.$emit('handlerVailable', true);
          this.validatePromises = [];
        })
        .catch(async (error) => {
          bol = false;
          // await this.$emit('handlerVailable', false);
          this.validatePromises = [];
        });
      return bol;
    },
