// Here is a list of the toolbar
// Detail list see https://www.tinymce.com/docs/advanced/editor-control-identifiers/#toolbarcontrols

// 经典模式
// const toolbar = ['code undo redo restoredraft | cut copy paste pastetext | forecolor backcolor bold italic underline strikethrough link anchor | alignleft aligncenter alignright alignjustify outdent indent | styleselect formatselect fontselect fontsizeselect | bullist numlist | blockquote subscript superscript removeformat | table image media charmap emoticons hr pagebreak insertdatetime print preview | fullscreen | bdmap indent2em lineheight formatpainter axupimgs importword']
// 精简模式
const toolbar = ['code undo redo | bold italic underline strikethrough link forecolor backcolor | alignleft aligncenter alignright alignjustify outdent indent lineheight | formatselect fontselect fontsizeselect | bullist numlist | blockquote removeformat | table image  | media charmap emoticons hr preview | fullscreen']
export default toolbar
