      ...
      <a-row>
        <a-col :span="12">
          <a-form-item label="账号状态" v-if="systemUser">
            ...
            >
              <a-select-option v-for="(status, index) in more.accountStatusOptions" :key="index" :value="status.value">
                {{ status.label }}
              </a-select-option>
            </a-select>
          </a-form-item>
            ...
            <a-input
              ...
              v-decorator="[
                'userNum',
                {
                  ...
                  rules: [
                    { required: true, message: '用户编号不能为空', whitespace: true },
                    { validator: this.checkUserNumDuplicate },
                    { max: 50, message: '50字符以内' },
                  ],
                },
              ]"
  ...
  methods: {
    async open(id, ...args) {
      ...
      if (id == null) {
        ...
        try {
          const res = await dibootApi.get('/iam/user/editShow', {
            id: id,
          });
          if (res.code === 0) {
            let models = res.data || {};
