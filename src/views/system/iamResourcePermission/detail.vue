<template>
  <a-drawer :title="title" width="720" :visible="visible" @close="close" :body-style="{ paddingBottom: '80px' }">
    <a-spin :spinning="spinning">
      <a-descriptions :column="2">
        ...
        <a-descriptions-item label="页面资源权限">
          <template v-if="!model.permissionList || model.permissionList.length === 0"> 无 </template>
        </a-descriptions-item>
      </a-descriptions>
      <a-tabs :default-active-key="0" v-if="model.permissionList && model.permissionList.length > 0">
        <a-tab-pane v-for="(p, i) in model.permissionList" :key="i" :tab="p.displayName">
          <a-descriptions :column="2">
