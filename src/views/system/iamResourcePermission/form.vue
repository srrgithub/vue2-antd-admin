<template>
  <a-drawer :title="title" width="1000" :visible="state.visible" @close="close" :body-style="{ paddingBottom: '80px' }">
    <a-row :gutter="5">
                  ...
                  <a-input
                    ...
                    v-decorator="[
                      'resourceCode',
                      {
                        ...
                        rules: [{ required: true, message: '菜单编码不能为空', whitespace: true }, { validator: this.checkCodeDuplicate }],
                      },
                    ]"
                  />
                </a-form-item>
              ...
              <a-col :span="24">
                <a-card size="small" title="按钮权限接口">
                  <a-button-group slot="extra">
                    <a-button @click="addNewPermission" type="primary" size="small" icon="plus"></a-button>
                    <a-button
                  ...
                  <a-tabs v-if="permissionList.length > 0" :defaultActiveKey="0" v-model="currentPermissionActiveKey">
                    <a-tab-pane v-for="(permission, index) in permissionList" :tab="permission.displayName" :key="index">
                      <a-form-item :required="true" label="按钮权限编码">
                        <a-row type="flex" align="middle" :gutter="16">
                          ...
                          <a-col :span="8">
                            <a-button type="primary" icon="swap" size="small" @click="handleSwap(permission, index)">{{
...
import { routersFormatter, treeList2list, treeListFormatter } from '@/utils/treeDataUtil';
...
export default {
  ...
  methods: {
    ...
    goPermissionConfig(configCode) {
      ...
      configCode === 'Menu' ? this.change2MenuPermissionConfig() : this.changePermissionConfig(this.permissionList[this.currentPermissionActiveKey]);
    },
