  ...
  <a-card :border="false">
        ...
        <a-row :gutter="48">
          <a-col :md="5" :sm="24">
            <a-form-item label="菜单名称">
              <a-input @keyup.enter.native="onSearch" allowClear placeholder="名称" v-model="queryParam.displayName" />
            </a-form-item>
          </a-col>
          <a-col :md="5" :sm="24">
            <a-form-item label="菜单编码">
              <a-input @keyup.enter.native="onSearch" allowClear placeholder="菜单编码" v-model="queryParam.resourceCode" />
            </a-form-item>
          </a-col>
          <a-col :md="5" :sm="24">
          ...
          <a-col :md="9" :sm="24" style="text-align: right">
            <a-button v-action:sort @click="$refs.sort.open()" icon="drag" type="default">排序</a-button>
            <a-button v-action:create type="primary" icon="plus" @click="create">新增</a-button>
          </a-col>
        </a-row>
      </a-form>
    </div>
    <a-alert
      v-if="!loadingData && (!data || data.length === 0)"
      message="请点击右上角新增开始配置系统菜单与权限！"
      banner
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <span v-permission-missing="['detail', 'update', 'delete']"> - </span>
      </span>
    </a-table>
...
export default {
  ...
  data() {
    return {
      ...
      // customQueryParam: { displayType: 'PERMISSION' },
      getOrderParamFromMixin: false,
